...
import de.tubs.cs.ias.appanalyzer.appbinary.{Analysis, MobileApp, ipa}
import de.tubs.cs.ias.appanalyzer.platform.frida.{FridaScripts, iOSFridaScripts}
import de.tubs.cs.ias.appanalyzer.platform.PlatformOS.{PlatformOS, iOS}
import de.tubs.cs.ias.appanalyzer.platform.exceptions.{AppClosedItself, UnableToInstallApp, UnableToUninstallApp}
...
case class iOSDevice(conf: Config) extends Device with LogSupport {
  ...
  override def getAppPackageAnalysis(conf: Config): Analysis = ipa.IPA(conf)
