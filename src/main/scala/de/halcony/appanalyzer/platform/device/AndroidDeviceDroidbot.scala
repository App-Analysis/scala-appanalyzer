...
class AndroidDeviceDroidbot(conf: Config, device: Option[String])
  ...
  *
  * @return true if the device has booted successfully, false otherwise
  */
override def checkBootState(): Boolean
  ...
  /** restart the phone.
  *
  * Reboots the device using ADB, waits for it to boot, and performs an unlock action
  *
  * @return true if the device restarted and booted successfully, otherwise throws a FatalError
  */
  ...
  *
  * For Droidbot, no additional reset operations are required since Frida and objection are not used
  */
  ...
  * @param noAppStartCheck if true, skips checking whether the app has started successfully
  ...
  *
  * @param path the path to the app binary
